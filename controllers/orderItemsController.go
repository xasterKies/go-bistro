...
func ItemsByOrder(id string) (orderItems []primitive.M, arr error) {
	...
	lookupTableStage := bson.D{{"$lookup", bson.D{{"from", "table"}, {"localField", "order.table_id"}, {"foreingField", "table_id"}, {"as", "table"}}}}
	...
	groupStage := bson.D{{"$group", bson.D{{"_id", bson.D{{"order_id", "$order_id"}, {"table_id", "$table_id"}, {"table_number", "$table_number"}, {"payment_due", bson.D{{"$sum", "$amount"}, {"total_count", bson.D{{"$sum", 1}}}, {"order_item"} }}}}}}
} 
